...
function ensureProviderReady(provider) {
  if (provider === 'mysql' && !mysqlReady) {
    ...
    const error = new Error(`MySQL is not connected. Check MYSQL_* settings and mysql2 installation.${detail}`);
  ...
  if (provider === 'mongodb' && !mongoReady) {
    ...
    const error = new Error(`MongoDB is not connected. Check MONGODB_* settings.${detail}`);
    error.statusCode = 503;
    throw error;
  }
}
