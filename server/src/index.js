...
const DB_COMPARE_PROVIDERS = process.env.DB_COMPARE_PROVIDERS || 'mongodb,mysql';
const DB_WRITE_PROVIDERS = process.env.DB_WRITE_PROVIDERS || DB_COMPARE_PROVIDERS || DB_PROVIDER;
...
function formatProviderError(provider, error) {
  ...
  if (provider === 'mysql' && ['ER_ACCESS_DENIED_ERROR', 'ER_DBACCESS_DENIED_ERROR'].includes(error.code)) {
    const { host, user, database } = getMySqlConfig();
    return `MySQL login failed for user ${user} from this machine to ${host}. Check MYSQL_USER/MYSQL_PASSWORD, allow this client IP in MySQL, and grant access to database ${database}.`;
  }
...
async function upsertPlan(payload, provider = DB_PROVIDER) {
  ensureProviderReady(provider);
...
async function upsertUser(reqBody, provider = DB_PROVIDER) {
  ensureProviderReady(provider);
...
async function upsertUserAcrossProviders(reqBody) {
  ...
  if (!writeProviders.includes(DB_PROVIDER)) {
    writeProviders.unshift(DB_PROVIDER);
  }

  const results = {};
  let primaryResult = null;

  for (const provider of writeProviders) {
    try {
      await initializeProvider(provider, provider === DB_PROVIDER);
      const savedUser = await upsertUser(reqBody, provider);
      results[provider] = { ok: true };

      if (provider === DB_PROVIDER) {
        primaryResult = savedUser;
      }
    } catch (error) {
      const formattedError = formatProviderError(provider, error);
      providerErrors[provider] = formattedError;
      results[provider] = { ok: false, message: formattedError };
  ...
  const responseUser = primaryResult?.toObject ? primaryResult.toObject() : primaryResult;
...
app.get('/api/health', (_req, res) => {
  res.json({
    ...
    providerErrors,
    writeProviders: getProviderList(DB_WRITE_PROVIDERS),
  });
});
...
app.post('/api/memberships', asyncRoute(async (req, res) => {
  return res.status(201).json(await upsertUserAcrossProviders(req.body));
}));
...
async function startServer() {
  ...
  const compareProviders = getProviderList(DB_COMPARE_PROVIDERS);
  const writeProviders = getProviderList(DB_WRITE_PROVIDERS);

  await Promise.all([...new Set([...compareProviders, ...writeProviders])].map((provider) => initializeProvider(provider, false)));
  ...
  app.listen(PORT, () => {
    ...
    console.log(`Write providers: ${getProviderList(DB_WRITE_PROVIDERS).join(', ')}`);
  });
}
